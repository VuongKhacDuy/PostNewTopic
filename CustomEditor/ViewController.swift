...
//  CustomEditor
//
//  Created by hahalolo on 4/10/20.
...
//    @IBOutlet weak var tagUserView: TagUser!
    
//    @IBOutlet weak var coverExperience: UIImageView!
    ...
    private var richTextView: TextView {
        get {
//            editorView.richTextView.backgroundColor = UIColor.green
    ...
    override func viewDidLoad() {
        ...
        experienceView.addSubview(editorView )
        experienceView.addSubview(titleTextView)
        experienceView.addSubview(titlePlaceholderLabel)
        experienceView.addSubview(separatorView)
        
        editorView.richTextView.textContainer.lineFragmentPadding = 0
        // color setup
        if #available(iOS 13.0, *) {
            experienceView.backgroundColor = UIColor.systemBackground
            ...
            experienceView.backgroundColor = UIColor.white
        }
    ...
    func updateScrollInsets() {
        ...
        var rightMargin = (experienceView.frame.maxX - editorView.frame.maxX)
        rightMargin -= experienceView.safeAreaInsets.right
    ...
    func updateTitleHeight() {
        let layoutMargins = experienceView.layoutMargins
        ...
        if titleWidth <= 0 {
            ...
            titleWidth = experienceView.frame.width - (insets.left + insets.right + layoutMargins.left + layoutMargins.right)
        }
    ...
    private func configureConstraints() {
        ...
        titleTopConstraint = titleTextView.topAnchor.constraint(equalTo: experienceView.topAnchor, constant: 0)
        ...
        let layoutGuide = experienceView.readableContentGuide
        ...
        NSLayoutConstraint.activate([
            ...
            editorView.topAnchor.constraint(equalTo: experienceView.topAnchor, constant: 0),
            editorView.bottomAnchor.constraint(equalTo: experienceView.bottomAnchor, constant: 0)
        ])
        
//        // constraint for view tag User
//        NSLayoutConstraint.activate([
//            tagUserView.leadingAnchor.constraint(equalTo: layoutGuide.leadingAnchor),
//            tagUserView.trailingAnchor.constraint(equalTo: layoutGuide.trailingAnchor),
//            tagUserView.heightAnchor.constraint(equalToConstant: 50) ,
//            tagUserView.bottomAnchor.constraint(equalTo: layoutGuide.bottomAnchor, constant: 0)
//        ])
    }
    ...
    fileprivate func refreshInsets(forKeyboardFrame keyboardFrame: CGRect) {
        ...
        // possibility that there could be other views on top or below the text experienceView.
        // keyboardInset is basically the distance between the top of the keyboard
        // and the bottom of the text experienceView.
        let localKeyboardOrigin = experienceView.convert(keyboardFrame.origin, from: nil)
        let keyboardInset = max(experienceView.frame.height - localKeyboardOrigin.y, 0)
    ...
    override var keyCommands: [UIKeyCommand] {
        ...
        if richTextView.isFirstResponder {
            return [UIKeyCommand(input:"B", modifierFlags: .command, action:#selector(toggleBold), discoverabilityTitle:NSLocalizedString("Bold", comment: "Discoverability title for bold formatting keyboard shortcut.")),
                     ...
                     UIKeyCommand(input:"U", modifierFlags: .command, action:#selector(ViewController.toggleUnderline(_:)), discoverabilityTitle: NSLocalizedString("Underline", comment:"Discoverability title for underline formatting keyboard shortcut.")),
...
extension ViewController : Aztec.TextViewFormattingDelegate {
...
extension ViewController {
    func handleAction(for barItem: FormatBarItem) {
        ...
        // function for each icon onn toolbar
    ...
    @objc func toggleTagUser(){
        richTextView.insertText("Tạm thời thì chạy dc r đó. ")
    ...
    func showLinkDialog(forURL url: URL?, text: String?, target: String?, range: NSRange, allowTextEdit: Bool = true) {
        ...
        alertController.addTextField(configurationHandler: { [weak self]textField in
            ...
            textField.addTarget(self,
                                action:#selector(ViewController.alertTextFieldDidChange),
    ...
    func createToolbar() -> Aztec.FormatBar {
        ...
        if #available(iOS 13.0, *) {
            ...
            toolbar.selectedTintColor = experienceView.tintColor
            toolbar.disabledTintColor = .lightGray
            toolbar.dividerTintColor = .gray
        }
        
        toolbar.overflowToggleIcon = .gridicon(.ellipsis)
        toolbar.frame = CGRect(x: 0, y: 0, width: experienceView.frame.width, height: 44.0)
    ...
    var scrollableItemsForToolbar: [FormatBarItem] {
        ...
        // make toolbar button for editor
        return [
...
extension FormattingIdentifier {
    ...
    private func gridicon(_ gridiconType: GridiconType) -> UIImage {
        let size = ViewController.Constants.formatBarIconSize
...
// extension FormattingIndentifier
extension FormattingIdentifier{
